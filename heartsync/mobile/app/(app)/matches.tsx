...
interface Match {
  ...
  other_user: { id: string; name: string; photos: string[]; city?: string };
  last_message?: string;
...
export default function MatchesScreen() {
  ...
  const renderItem = ({ item }: { item: Match }) => (
    <TouchableOpacity style={styles.card} onPress={() => router.push(`/(app)/chat/${item.id}`)}>
    ...
    <View style={styles.screen}>
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptyText}>Continua a scorrere i profili!</Text>
        </View>
      ) : (
        <FlatList
          data={matches}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 26, fontWeight: "900", color: "#1a1a2e", paddingHorizontal: 20, paddingTop: 56, paddingBottom: 16 },
